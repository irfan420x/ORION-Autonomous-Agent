...
class DebugCommand(BaseModel):
    ...
    args: Dict[str, Any] = Field({}, description="Arguments for the debug command")
...
class MemorySnapshot(BaseModel):
    session_memory: Dict[str, Any] = Field({}, description="Snapshot of session memory content")
    working_memory_summary: str = Field(..., description="Summary of working memory")
    long_term_memory_stats: Dict[str, Any] = Field({}, description="Statistics about long-term memory")
    semantic_memory_stats: Dict[str, Any] = Field({}, description="Statistics about semantic memory")
...
class WorkflowVisualization(BaseModel):
    ...
    highlighted_tasks: List[str] = Field([], description="List of task IDs to highlight")
...
class PromptTemplate(BaseModel):
    ...
    variables: List[str] = Field([], description="List of variables used in the template")
