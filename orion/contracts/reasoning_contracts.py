...
class ProblemStatement(BaseModel):
    ...
    context: Dict[str, Any] = Field(default_factory=dict, description="Relevant context for the problem")
...
class Solution(BaseModel):
    ...
    steps: List[str] = Field(default_factory=list, description="Steps to execute the solution")
    confidence: float = Field(..., description="Confidence score (0.0-1.0) in the solution")
    reasoning_path: List[str] = Field(default_factory=list, description="Path of reasoning taken to arrive at the solution")
...
class InferenceResult(BaseModel):
    ...
    supporting_facts: List[str] = Field(default_factory=list, description="Facts or evidence supporting the inference")
...
class ReflectionReport(BaseModel):
    ...
    lessons_learned: List[str] = Field(default_factory=list, description="Key lessons learned from the task execution")
    identified_errors: List[str] = Field(default_factory=list, description="Errors or inefficiencies identified")
    suggested_improvements: List[str] = Field(default_factory=list, description="Suggestions for future improvements")
